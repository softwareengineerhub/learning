...
public class OrderTwoStream {
    ...
    public KStream<String, Person> kstreamCommoodityTradingBranch(StreamsBuilder builder){
        ...
        KStream<String, PersonPattern>[] personPatternStreamArray = personStream.mapValues(p->Helper.mapToPersonPattern(p)).branch(new Predicate<String, PersonPattern>() {
            @Override
            public boolean test(String s, PersonPattern personPattern) {
                return "Piter".equalsIgnoreCase(personPattern.getPatternName());
            }
        }, new Predicate<String, PersonPattern>() {
            @Override
            public boolean test(String s, PersonPattern personPattern) {
                return true;
            }
       ...
       // System.out.println("###########personPatternStreamArray="+personPatternStreamArray.length);
        personPatternStreamArray[0].to("t.commodity.plastic", Produced.with(stringSerde, personPatternSerde));
        personPatternStreamArray[1].to("t.commodity.nonplastic", Produced.with(stringSerde, personPatternSerde));
